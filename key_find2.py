...
print("Trying different matrix elements...")
# 只使用第一行和第一列的元素，因为这些元素不太可能都是0
for i in range(5):
    try:
        key = discrete_log(D[0,i], C[0,i], bounds=(2^62, p))
        found_keys.add(key)
        print(f"Found potential key from first row element ({0},{i}): {key}")
    except Exception:
        continue
        
    try:
        key = discrete_log(D[i,0], C[i,0], bounds=(2^62, p))
        found_keys.add(key)
        print(f"Found potential key from first column element ({i},0): {key}")
    except Exception:
        continue

print(f"\nFound {len(found_keys)} unique potential keys")
keys_list = sorted(list(found_keys))

# 测试每个可能的key
for k in keys_list:
    print(f"\nTesting key: {k}")
    try:
        Ck = C^k  # 在Sage中，矩阵幂运算使用^而不是**
        if Ck == D:
            print("Found valid key! Matrix validation successful!")
            print(f"Key = {k}")
            exit(0)
